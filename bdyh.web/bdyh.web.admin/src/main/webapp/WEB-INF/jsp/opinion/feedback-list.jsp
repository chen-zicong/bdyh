...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> </span> <span class="r">共有数据：<strong>${userOpinionList.size() }</strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				<tr class="text-c">
					...
					<th>ID</th>
					<th>用户名</th>
					<th width="600">意见内容</th>
					<th>反馈时间</th>
					<th>操作</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${userOpinionList }" var="userOpinion">
					<tr class="text-c">
						<td><input type="checkbox" value="1" name=""></td>
						<td>${userOpinion.uoid }</td>
						<th>${userOpinion.userWechat.nickname }</th>
						<th>
							<div>${userOpinion.opinion }</div>
						</th>
						<td>${userOpinion.feedbackTime }</td>
						<td class="td-manage"> <a title="删除" href="javascript:;" onclick="opinion_del(this,'${userOpinion.uoid}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
