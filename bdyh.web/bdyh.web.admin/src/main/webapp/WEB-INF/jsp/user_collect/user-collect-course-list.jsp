...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 收藏记录 <span class="c-gray en">&gt;</span> 收藏课程列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20">  <span class="r">共有数据：<strong>${userCollectCourseList.size() }</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${userCollectCourseList }" var="userCollectCourse">
				<tr class="text-c">
					<th>${userCollectCourse.userWechat.nickname }</th>
					<td>
						<c:choose>
							<c:when test="${userCollectCourse.userWechat.sex eq 1 }">男</c:when>
							<c:otherwise>女</c:otherwise>
						</c:choose>
					</td>
					<td><img width="100" height="100" class="picture-thumb" src="${userCollectCourse.userWechat.headimgurl }"></td>
					
					<th>${userCollectCourse.course.courseName }</th>
					<td><img width="150" height="100" class="picture-thumb" src="http://bdpak.cn:8080/home/courseImg/${userCollectCourse.course.courseImg }"></td>
					<td>${userCollectCourse.course.courseDesc }</td>
