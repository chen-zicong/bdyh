...
<article class="page-container">
	<form action="${pageContext.request.contextPath}/adminAgent/agentAdd" class="form form-horizontal" id="form-agent-add" method="POST">
		<input hidden="hidden" name="agentId" value="${agent.agentId }">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>代理商姓名：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${agent.nickname }" placeholder="" id="nickname" name="nickname">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${agent.telephone }" placeholder="" id="telephone" name="telephone">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>邮箱：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="@" name="email" id="email" value="${agent.email }">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${agent.username }" placeholder="" id="username" name="username">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="agentIntro" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" onKeyUp="$.Huitextarealength(this,100)">${agent.agentIntro }</textarea>
...
$(function(){
	...
	$("#form-agent-add").validate({
		...
		submitHandler:function(form){
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/adminAgent/agentEdit",
				type : "post",
				data : fd,
				dataType : "json",
				processData : false, // 告诉jQuery不要去处理发送的数据
